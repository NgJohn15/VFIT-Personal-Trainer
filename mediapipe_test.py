...
with mp_pose.Pose(
  ...
  while cap.isOpened():
    ...
    verti = cv2.resize(verti, (320,360), interpolation=cv2.INTER_CUBIC) #INTER_CUBIC interpolation
    #cv2.imshow('Test View', verti)
    hori = np.concatenate((image[:,120:520], verti), axis=1)
    hori = cv2.resize(hori, (1920,1080), interpolation=cv2.INTER_CUBIC) #INTER_CUBIC interpolation
    cv2.putText(img=hori, text=fps, org= (0, 70), fontFace=cv2.FONT_HERSHEY_DUPLEX, fontScale=2, color=(0, 255, 0),thickness=1)
